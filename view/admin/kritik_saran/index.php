<?php $menu_aktif = 'kritik'; require_once BASE_PATH . '/view/admin/layout_admin_header.php'; ?>
...
<?php if (empty($semua_pesan)): ?>
  <div class="text-center py-5 text-muted"><i class="bi bi-chat-x fs-1 d-block mb-2"></i>Belum ada pesan masuk.</div>
<?php else: ?>
<div class="row g-3">
  ...
  <div class="col-md-6 col-lg-4">
    <div class="card border-0 shadow-sm h-100">
      <div class="card-body">
        ...
        <?php if ($p['email']): ?>
          <small class="text-muted"><i class="bi bi-envelope me-1"></i><?= htmlspecialchars($p['email']) ?></small><br/>
        <?php endif; ?>
        <p class="mt-2 mb-2 small"><?= nl2br(htmlspecialchars($p['pesan'])) ?></p>
        <div class="d-flex justify-content-between align-items-center mt-2">
          <small class="text-muted"><?= date('d M Y, H:i', strtotime($p['created_at'])) ?></small>
          <a href="<?= BASE_URL ?>/aksi/hapus-kritik?id=<?= $p['id'] ?>" class="btn btn-outline-danger btn-sm"
            onclick="return confirm('Hapus pesan ini?')"><i class="bi bi-trash"></i></a>
        </div>
      </div>
    </div>
  </div>
